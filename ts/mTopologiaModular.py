...
def dados(oid):
    ...
    dados = f"""<mxCell id="{oid}" value="&lt;div&gt;BLOCO IP: {blocoIp}&amp;nbsp;&amp;nbsp;&lt;/div&gt;&lt;div&gt;IP: {ip}&amp;nbsp;&lt;/div&gt;&lt;div&gt;GATEWAY: {gateway}&amp;nbsp;&lt;/div&gt;&lt;div&gt;MASCARA: {mascara}&lt;/div&gt;&lt;div&gt;VLAN: {vlan}&amp;nbsp;&lt;/div&gt;&lt;div&gt;BANDA: {banda} Mbps&lt;/div&gt;" style="text;html=1;align=left;verticalAlign=middle;whiteSpace=wrap;rounded=0;" parent="1" vertex="1">
          <mxGeometry x="-100" y="120" width="260" height="120" as="geometry" />
        </mxCell>"""
    
    return dados

def titulo(oid, nome,equipamentoA,equipamentos):
    posX = equipamentoA.posX
    
    titulo = f"""<mxCell id="{oid}" value="{nome}" style="text;html=1;align=center;verticalAlign=middle;rounded=0;labelBackgroundColor=none;textShadow=0;whiteSpace=wrap;strokeColor=light-dark(#ffffff, #a9b8c7);" parent="1" vertex="1">
...
def gerarIdentificacao(nome):
    
    ids = f"{formatarTexto(nome)}"
    return ids  
...
# Adicionar: """)
    ...
    nomeArquivo = gerarIdentificacao(nome)
    
    meio = meio + dados(f"DADOS-{iid}")
    iid = iid + 1
    meio = meio + titulo(f"TITULO-{iid}", nome, equipamentoCentral(equipamentos),equipamentos)
    iid = iid + 1
